(function () {
    ...
    function App() {
        ...
        this.clickConfigs = function (e) {
            ...
            if (target.className.indexOf("li") > -1) {
                ...
                self.activeJson = self.data[target.getAttribute("id")];
        ...
        this.getStr = function (json) {
            ...
            json.style.background && strArr.push(
                "背景颜色：" + "<span style='padding: 0 5px;background:" + json.style.background + "'></span>;" || "");
            json.text && strArr.push(
                "<p>内容：" + json.text.substr(0, 20) + (json.text.length > 20 ? "..." : "") + "</p>");
    ...
    function Phone() {
        ...
        this.setPhone = function (el, json) {
            ...
            for (s in json.style) {
                if (json.style.hasOwnProperty(s)) {
                    newEl.style[s] = json.style[s];
                }
            }
            el.appendChild(newEl);
            if (json.type !== "img") {
            ...
            if (json["img-list"]) {
                newEl.setAttribute("img-list", json["img-list"]);
            }
            if (!json.content) {
                return;
            }
    ...
    function Win() {
        ...
        this.setValues = function () {
            ...
            for (var i = 0; i < l; i++) {
                ...
                if (_name) {
                    if (_name === "img-list") {
                        this.values[i].value = JSON.parse(this.data[_name] || "[]").join(",") || "";
                    } else {
                        this.values[i].value = this.data[_name] || "";
                    }
                }
                if (_style) {
                    this.values[i].value = this.data.style[_style] || "";
                }
            }
        };
        this.sub = function () {
            ...
            for (var i = 0; i < l; i++) {
                ...
                var _style = this.values[i].getAttribute("v-style");
                if (_name) {
                    this.data[_name] = this.values[i].value || "";
                }
                if (_style) {
                    this.data.style[_style] = this.values[i].value || "";
                }
            }
            if (!this.data.type) {
                alert("请选择节点类型");
            ...
            if (this.data["img-list"]) {
                this.data["img-list"] = JSON.stringify(this.data["img-list"].split(","));
            }
            this.data.content = this.data.content || [];
            tools.setData(this.data, userData);
            this.hide();
        };
        this.initVlaue = function () {
            var l = this.values.length;
            for (var i = 0; i < l; i++) {
                this.values[i].value = "";
            }
